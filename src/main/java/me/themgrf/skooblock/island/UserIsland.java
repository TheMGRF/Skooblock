...
import me.themgrf.skooblock.island.roles.IslandMemberDefaultRole;
...
public class UserIsland implements Comparable<UserIsland> {
    ...
    private final String owner, centre, lowerBound, upperBound, home, description;
    private String biome, warp;
    ...
    public void generateConfig(UserIsland userIsland) {
        main.getConfigManager().createUserConfig(userIsland.owner, userIsland.centre, userIsland.lowerBound, userIsland.upperBound, userIsland.biome, userIsland.home, userIsland.warp, userIsland.settings, userIsland.size, userIsland.level);
    }
    ...
    public void addMember(String uuid) {
        FileConfiguration config = main.getConfigManager().getUserConfig(getOwner());
        config.set("members." + uuid, "default");
        try {
            config.save(main.getConfigManager().getUserFile(getOwner()));
            members.add(uuid);
        } catch (IOException e) {
            e.printStackTrace();
        }
